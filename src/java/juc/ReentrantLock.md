...
ReentrantLock 实现了Lock接口，Lock接口中定义了 lock 与 unlock 相关操作，并且还存在newCondition方法，表示生成一个条件。
...
![image](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102022394.png)

说明: ReentrantLock类内部总共存在`Sync`、`NonfairSync`、`FairSync`三个类，`NonfairSync`与`FairSync`类继承自Sync类，Sync类继承自AbstractQueuedSynchronizer抽象类。下面逐个进行分析。
...
![image](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102040307.png)
...
![image](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102042447.png)
...
![image](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102043935.png)
...
![image](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102043107.png)

说明: 由调用流程可知，t1线程成功获取了资源，可以继续执行。

- t2线程执行lock.lock，下图给出了方法调用中的主要方法。

![image](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102043489.png)

说明: 由上图可知，最后的结果是t2线程会被禁止，因为调用了LockSupport.park。

- t3线程执行lock.lock，下图给出了方法调用中的主要方法。

![image](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102043089.png)

说明: 由上图可知，最后的结果是t3线程会被禁止，因为调用了LockSupport.park。

- t1线程调用了lock.unlock，下图给出了方法调用中的主要方法。

![image](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102043937.png)

说明: 如上图所示，最后，head的状态会变为0，t2线程会被unpark，即t2线程可以继续运行。此时t3线程还是被禁止。

- t2获得cpu资源，继续运行，由于t2之前被park了，现在需要恢复之前的状态，下图给出了方法调用中的主要方法。

![image](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102043087.png)
...
![image](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102044561.png)

说明: 由上图可知，最终unpark t3线程，让t3线程可以继续运行。

- t3线程获取cpu资源，恢复之前的状态，继续运行。

![image](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102044040.png)

说明: 最终达到的状态是sync queue中只剩下了一个结点，并且该节点除了状态为0外，其余均为null。

- t3执行lock.unlock，下图给出了方法调用中的主要方法。

![image](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102044334.png)
