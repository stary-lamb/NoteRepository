...
#### 方法锁形式：synchronized修饰普通方法，锁对象默认为 this
...
<img src="https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102111305.png" alt="img" style="zoom: 33%;" />
...
![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102112389.png)
...
<img src="https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102112166.png" alt="img" style="zoom: 67%;" />
...
在 JDK1.6及之前的版本中，synchronized锁是通过对象内部的一个叫监视器锁（也称对象锁）来实现的。当一个线程请求对象锁时，如果该对象没有被锁住，线程就会获取锁并继续执行。如果该对象已经被锁住，线程就会进入阻塞状态，直到锁被释放。这种锁的实现方式称为**“重量锁”**，因为获取锁和释放锁都需要在操作系统层面上进行线程的阻塞和唤醒，而这些操作会带来很大的开销。

在 JDK1.6之后，synchronized锁的实现发生了一些变化，引入了“偏向锁”,“轻量级锁" 和“重量级锁”三种不同的状态，用来适应不同场景下的锁竞争情况。

所以，在 Java 中，锁的状态分为四种，分别是无锁状态、偏向锁状态、轻量级锁状态和重量级状态。在 Java 中，`mark word` 的低两位用于表示锁的状态，分别为 “01”（无锁状态）、“01”（偏向锁状态）、“00”（轻量级状态）、“10”（重量级状态）。但是由于无锁状态和偏向锁状态都是“01”，所以在低三位引入偏向锁标记位，用“0”表示无锁，“1”表示偏向锁。**这几种状态会随着竞争的情况逐渐升级，但不能降级，目的是为了提高锁和释放锁的效率。**

![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202307181859849.png)



### 无锁

一个线程第一次访问一个对象的同步块时，JVM会在对象头中设置该线程的Thread ID，并将对象头的状态位设置位“偏向锁”。这个这个过程称为“偏向”，表示对象当前偏向于第一个访问它的线程。
...
![image-20230718190727042](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202307181907161.png)
...
2. 如果替换成功，整个同步过程就完成了。
3. 如果替换失败，说明其他线程尝试过获取该锁（此时锁已经膨胀），那就要释放锁的同时，唤醒被挂起线程。

![image-20230720111052973](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202307201111046.png)
...
![锁升级](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202307201115746.png)
...
<img src="https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102113234.png" alt="img" style="zoom: 67%;" />
...
![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102114993.png)
...
![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102114743.png)
...
![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102114851.png)
...
![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102114226.png)
...
<img src="https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102115179.png" alt="img" style="zoom: 85%;" />
...
![image-20230610211650784](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102116865.png)
