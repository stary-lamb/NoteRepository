...
![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102052630.png)
...
![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102059968.png)
...
![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102059465.png)
...
![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102100549.png)
...
![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102100525.png)
...
![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102055155.png)

- rt1线程执行rrwLock.readLock().lock操作，主要的函数调用如下。

![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102055485.png)

说明: 此时，AQS的状态state为2^16 次方，即表示此时读线程数量为1。

- rt2线程执行rrwLock.readLock().lock操作，主要的函数调用如下。

![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102055359.png)

说明: 此时，AQS的状态state为2 * 2^16次方，即表示此时读线程数量为2。

- wt1线程执行rrwLock.writeLock().lock操作，主要的函数调用如下。

![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102056234.png)

说明: 此时，在同步队列Sync queue中存在两个结点，并且wt1线程会被禁止运行。

- rt1线程执行rrwLock.readLock().unlock操作，主要的函数调用如下。

![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102056310.png)

说明: 此时，AQS的state为2^16次方，表示还有一个读线程。

- rt2线程执行rrwLock.readLock().unlock操作，主要的函数调用如下。

![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102056239.png)
...
![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102056514.png)

说明: 最后，sync queue队列中只有一个结点，并且头节点尾节点均指向它，AQS的state值为1，表示此时有一个写线程。

- wt1执行rrwLock.writeLock().unlock操作，主要的函数调用如下。

![img](https://qijiayi-image.oss-cn-shenzhen.aliyuncs.com/img/202306102056376.png)
