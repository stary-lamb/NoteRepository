...
首先每一个控制块都有两个指针，用来关联空首先每个控制块都有两个指针，用来关联空闲的上下两个控制块。InnoDB再定义了一个free控制块的**基点**，将所有空闲的控制块串联成双向链表。
...
我们知道hash的查询速度是最快的O（1），而b+树可能还需要三层四层，如果是二级索引，还需要回表，特别慢。 Innodb存储引擎会监控对表上个索引页的查询。
